...
def test_online_extraction_rejects_open_loop_top_down_model() -> None:
    contract = ModelContract(
        ...
        encoder_family="gru",
        top_down_source="level_3:encoder.place_codes",
        top_down_mode="aligned",
        top_down_context_dim=4,
    )

    with pytest.raises(
        ValueError,
        match="does not support open-loop top-down DAG checkpoints",
    ):
